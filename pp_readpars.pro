...
;Parse the lines
ret = CREATE_STRUCT("DuneCrestsConfigFile", 1)
...
FOR i = 0, N_ELEMENTS(lines) - 1 DO BEGIN
  ; Get the line out of the big array of lines
  line = lines[i]
  if STRCMP(line, "#", 1) NE 1 then begin ;Skip lines that are entirely comments, or are empty
    comment_split = strsplit(line, comment_marker, /extract, count=count)
    if count GT 0 then nc_line = comment_split[0] else continue
    tmp=strsplit(nc_line,value_separator,/extract,count=count)
    ...
    ret = CREATE_STRUCT(ret, key, value)
  endif
ENDFOR
return,ret
end